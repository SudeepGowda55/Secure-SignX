...
run(async (context) => {
  ...
  if (text.includes("Document Submission:")) {
    ...
    if (senderAddress === complianceOfficerAddress) {
      ...
      await context.sendTo(`Your document "${documentData.document_name}" has been ${newStatus}.`, [
    ...
    if (senderAddress === complianceOfficerAddress) {
      ...
      await context.send("You are not authorized to view the pending documents.");
    }
  } else if (text.toLowerCase() === "view reports") {
    if (senderAddress === managerAddress) {
      await generateReports(context);
    } else {
      await context.sendTo("You are not authorized to view reports.", [senderAddress]);
    }
